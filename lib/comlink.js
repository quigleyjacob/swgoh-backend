...
class Comlink {
    ...
    constructor() {
        this.url = process.env.COMLINK_URL
        this.statsURL = process.env.STATS_URL
    }
    ...
    // payload either contains items or requestSegment field, if requestSegment is provided it will get the entire segment from comlink, otherwise it will get the specific items provided in the items field
    async getGameData(payload, latestGamedataVersion = undefined) {
        let body = {
            "payload": {
