...
class GG {
    ...
    async insertGACHistory(allyCode, gacNumber, roundNumber, report={added:0,failed:0}) {
        try {
            ...
            report.added += 1
            return 'GAC Data Added'
        } catch(err) {
            console.log(err)
            report.failed += 1
            return 'Cannot add GAC Data'
        }
    }
    ...
    async addEntireHistory(allyCode) {
        ...
        await Promise.all(range(this.firstGAC, latest).map(async gacNumber => {
    ...
    async getGACBattles(filter) {
        return DB.getGGReports(filter)
    }
