...
class Gac {
    ...
    async generateGacBoardImage(discordId, allyCode, renderMode) {
            ...
            let counterLinks = Object.keys(formattedGacBoard?.awayStatus || {}).map(key => {
                ...
                let link = getRegularCounterUrl({enemySquadData: squad, isFleet, eventInstanceId})
            ...
            try {
                let now = Date.now()
                let key = `${tournamentEventId}/${allyCode}/${now}.webp`
                let compressed = await sharp(buffer)
                    .webp({quality: 75})
                    .toBuffer()
                let url = await uploadSnapshot(compressed, key)
