...
import { drawFullBoard } from "../../canvas.js"
import { getInsightCounterUrl, getRegularCounterUrl } from "../../../utils/counterUrls.js"
import { uploadSnapshot } from "../../aws.js"
import { getCurrentGAC } from "../../hu.js"
import { formatHotUtilsGacBoard, formatMhannGacBoard, mergeGACPlans, loadGACBoardFromCustomEndpoint } from "../../../utils/gac.js"
...
class Gac {
    ...
    async getCurrentGACBoard(session, allyCode) {
        ...
        // return JSON.parse(fs.readFileSync(path.resolve(__dirname, './gac.json')))
        try {
                let user = await Session.sessionToDiscord(session)
                let userSettings = await DiscordUser.getSettings(user.id)
                let gacBoard
                if(userSettings && userSettings.gacEndpoint && (userSettings.gacEndpoint.enabled ?? true)) {
                    gacBoard = await loadGACBoardFromCustomEndpoint(userSettings.gacEndpoint, allyCode)
                } else {
                    gacBoard = await Mhann.getCurrentGACBoard(allyCode, user.id)
                }
    ...
    async generateGacBoardImage(discordId, allyCode, renderMode) {
            const response = await Mhann.getCurrentGACBoard(allyCode, discordId)

            const currentMatch = response.matchStatus.at(-1)
            let tournamentEventId = response.tournamentEventId

            let mode = currentMatch.tournamentMapId.includes('5v5') ? 5 : 3
            let gacInfo = await Data.getGacInfo(mode)
            let eventInstanceId = gacInfo.eventInstanceId.split(':')[0]

            let player = await Player.getPlayer({allyCode: allyCode})
            let opponentExists = currentMatch.opponent?.id
            let opponent = opponentExists ? await Player.getPlayer({playerId: currentMatch.opponent.id}) : undefined

            let characters = await Data.getUnits()

            let characterMap = characters.reduce((obj, unit) => {
                obj[unit.baseId] = unit
                return obj
            }, {})

            const datacronData = await Data.getData('datacron')
            let affixTextMap = datacronData.affixTextMap

            const buffer = await drawFullBoard(currentMatch, characterMap, player, opponent, renderMode)

            let formattedGacBoard = opponentExists ? await formatMhannGacBoard(response, allyCode, true) : undefined

            let counterLinks = Object.keys(formattedGacBoard?.awayStatus || {}).map(key => {
                let squad = formattedGacBoard.awayStatus[key]
                let isFleet = key.includes('4zone_phase02_conflict01_duel01')
                let leader = squad.squad[0].baseId

                let link = getRegularCounterUrl({squadData: squad, isFleet, eventInstanceId})
                let insightsLink = getInsightCounterUrl({enemySquadData: squad, opponentDatacrons: opponent.datacron, activeGac: formattedGacBoard, isFleet, affixTextMap, units: characters})

                return {link, insightsLink, text: characterMap[leader].nameKey}
            })

            try {
                let now = Date.now()
                let key = `${tournamentEventId}/${allyCode}/${now}.png`
                let url = await uploadSnapshot(buffer, key)
                return {url, links: counterLinks}
            } catch(err) {
                throw new MyError(400, 'unable to upload image', err)
            }
    }
    ...
    async loadGACPlanFromGameData(allyCode, body) {
        ...
        let newBoardData = await formatMhannGacBoard(body, allyCode)
        // return newBoardData
        let time = Date.now()
        newBoardData.time = time
        newBoardData.createdAt = time
        newBoardData.updatedAt = time
