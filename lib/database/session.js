...
class Session {
    ...
    async sessionInGuild(sessionId, guildId) {
        ...
        let guilds = await DB.getAccountsByDiscordId(user.id)
    ...
    async sessionIsGuildOfficer(sessionId, guildId) {
        ...
        let user = await this.sessionToDiscord(sessionId)
        let account = (await DB.getAccountsByDiscordId(user.id)).filter(account => account.guildId === guildId)
    ...
    async sessionIsPlayer(sessionId, allyCode) {
        let user = await this.sessionToDiscord(sessionId)
        let account = (await DB.getAccountsByDiscordId(user.id)).filter(account => account.allyCode === allyCode)
