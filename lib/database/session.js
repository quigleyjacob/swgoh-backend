...
class Session {
    ...
    async sessionInGuild(sessionId, guildId, allyCode = undefined) {
        let user = await this.sessionToDiscord(sessionId)
        let guilds = await DiscordUser.getAccountsByDiscordId(user.id)
        if(allyCode) {
            guilds = guilds.filter(guild => guild.allyCode === allyCode)
        }
        return guilds.map(guild => guild.guildId).includes(guildId)
    }

    async getGuildOfficerAllyCode(discordId, guildId) {
        let accountsInGuild = (await DiscordUser.getAccountsByDiscordId(discordId)).filter(account => account.guildId === guildId)
        if(process.env.DEV) {
            return accountsInGuild.find(account => account.primary) || accountsInGuild?.[0] || undefined
        }
        let guild = await Guild.getGuild(guildId)
        let officerAccounts = guild.member.filter(member => accountsInGuild.some(account => account.allyCode === member.allyCode) && member.memberLevel > 2)
        if(officerAccounts.length === 0) {
            return undefined
        }
        let primaryAccount = accountsInGuild.find(account => account.primary)
        if(primaryAccount && officerAccounts.find(account => account.allyCode === primaryAccount.allyCode)) {
            return primaryAccount
        }
        return accountsInGuild.find(account => account.allyCode === officerAccounts[0].allyCode)
    }
