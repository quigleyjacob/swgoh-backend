...
import { 
    ...
    defaultEraDefinitionProjection
...
class Data {
    ...
    async getUnits(projection = defaultUnitProjection) {
    ...
    async getAbilityList(idList, localizeField = 'descKey') {
        let match = idList ? {id: {$in: idList}} : {abilityType: 7}
        let abilityList = await this._get('ability', match, defaultAbilityProjection, localizeField)
        return abilityList
    }
    ...
    async getData(type) {
        let response = await this._getOne('data', {type}, null, null)
        if(!response) {
            return {}
        }
        return response
    ...
    async getCurrentEra(projection = defaultEraDefinitionProjection) {
        let now = Date.now().toString()

        return this._getOne('eraDefinition', { startTimeMs: {$lt: now}, endTimeMs: {$gt: now}}, projection)
    }

    async getRecipes(idList = []) {
        let match = idList.length > 0 ? {id: {$in: idList}} : {}
        return this._get('recipe', match, null, null)
    }

    async getMysteryBoxes(idList = []) {
        let match = idList.length > 0 ? {id: {$in: idList}} : {}
        return this._get('mysteryBox', match, null, null)
    }

    async getRaid(raidId) {
        return this._getOne('guildRaid', {id: raidId}, null)
    }

    async getTb(id) {
        return this._getOne('territoryBattleDefinition', {id}, null)
    }

    async getLocalization(regex) {
        return this._get('Loc_ENG_US', {key: RegExp(regex)})
    }

    async getCampaign(campaignId, campaignMapId, campaignNodeId) {
        let { db } = await connectToDatabase()
        let collectionName = 'campaign'
        let aggregationArray = [
                {
                    $match: {
                    id: campaignId
                ...
                // Pull out only the RAIDS campaign map
                {
                    $project: {
                    campaignMap: {
                        $filter: {
                ...
                // Unwind down to campaignNode
                {
                    $unwind: "$campaignMap"
                },
                {
                    $unwind: "$campaignMap.campaignNodeDifficultyGroup"
                },
                {
                    $unwind: "$campaignMap.campaignNodeDifficultyGroup.campaignNode"
                },
                // Filter for ORDER66
                {
                    $match: {
                    "campaignMap.campaignNodeDifficultyGroup.campaignNode.id": campaignNodeId
                    }
                },
                // Return ONLY the ORDER66 node
                {
                    $replaceRoot: {
                    newRoot: "$campaignMap.campaignNodeDifficultyGroup.campaignNode"
                    }
                }
                ]
        try {
            return (await db.collection(collectionName).aggregate(aggregationArray).toArray())[0]
        } catch(err) {
            throw handleDBError(err, collectionName, 'get')
        }
    }

    async getGacInfo(mode) {
        try {
            ...
            if(response.ok) {
                ...
                throw new MyError(500, "Failed to fetch GAC info", error)
            }
        } catch(err) {
            throw new MyError(500, "Failed to fetch GAC info", err)
        }
    }
