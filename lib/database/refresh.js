...
import { powerRankingMultiplier, itemMask } from "../../utils/constants.js"
import { getModScore } from "../../utils/mods.js"
import { zetaCount, omicronCount, buildUnitOmicronSummary } from "../../utils/units.js"
...
import { uploadSnapshot } from "../aws.js"
...
class Refresh {
    ...
    async refreshPlayer(payload, projection = defaultPlayerProjection) {
        try {
            ...
            let eraDefinition = await Data.getCurrentEra()
            ...
            comlinkResponse.rosterUnit.forEach(unit => {
                ...
                unit.omicron = buildUnitOmicronSummary(unit, skillMap)
            })
            ...
            // add era units to a new array
            comlinkResponse.eraRosterUnit = comlinkResponse.rosterUnit.filter(unit => {
                return eraDefinition.eraUnitId.includes(unit.baseId)
            })

            // remove era units from rosterList
            comlinkResponse.rosterUnit = comlinkResponse.rosterUnit.filter(unit => {
                return !eraDefinition.eraUnitId.includes(unit.baseId)
            })
    ...
    async refreshPlayers(array, projection, key, progressCallback = null) {
        let completed = 0
        const total = array.length
        const results = await Promise.allSettled(array.map(async id => {
            try {
                const player = await this.refreshPlayer({[key]: id}, projection)
                if (progressCallback) {
                    completed += 1
                    await progressCallback(completed, total, id)
                }
                return player
            } catch (err) {
                if (progressCallback) {
    ...
    async refreshGameData(latestGamedataVersion = undefined) {
        ...
        let partitionMap = {
            '1': ['material', 'equipment', 'battleTargetingRule', 'playerPortrait', 'skill', 'category', 'recipe'],
            '2': ['territoryBattleDefinition', 'ability', 'guildRaid'],
            '3': ['units', 'relicTierDefinition'],
            '4': ['datacronSet', 'datacronTemplate', 'datacronAffixTemplateSet', 'eraDefinition', 'mysteryBox', 'campaign']
        }
        ...
        for await (const requestSegment of Object.keys(partitionMap)) {
            ...
            // Calculate mask value for this partition
            let maskValue = partitionMap[requestSegment].reduce((mask, itemType) => {
                let itemMaskValue = BigInt(itemMask[itemType] || '0')
                return mask | itemMaskValue
            }, 0n)
            
            let comlinkResponse = await Comlink.getGameData({ items: maskValue.toString() }, gamedataVersion)
            ...
            for await (const objectName of partitionMap[requestSegment]) {
                ...
                if(objectName === 'campaign') {
                    objectData = objectData.filter(campaign => campaign.id === 'GUILD')
                }
    ...
    async refreshAssets(newAssetVersion, previousAssetVersion) {
        if(newAssetVersion === previousAssetVersion) {
            console.log('asset versions the same')
            return
        }
        let assetsURL = `http://swgoh_assets:8080/Asset/listDiff?version=${newAssetVersion}&diffVersion=${previousAssetVersion}&diffType=1`
        let newAssetPathsResponse = await fetch(assetsURL)
        if(newAssetPathsResponse.ok) {
            let newAssetPaths = await newAssetPathsResponse.json()
            let assets = await Promise.allSettled(newAssetPaths.map(async assetName => {
                const prefixes = ["ability", 'charui']
                let prefix = prefixes.find(p => assetName.startsWith(`${p}_`))
                if(!prefix) {
                    return
                }
                let getAssetURL = `http://swgoh_assets:8080/Asset/single?version=${newAssetVersion}&assetName=${assetName}`
                let response = await fetch(getAssetURL)
                if(!response.ok) {
                    return
                }
                let responseHeaders = response.headers.get('content-disposition')
                let filename = responseHeaders.match(/filename=([^;]+)/)[1]
                const buffer = Buffer.from(await response.arrayBuffer())
                await uploadSnapshot(buffer, `${prefix}/${filename}`, 'swgoh-images')
                return filename
            }))
            
            let success = assets.filter(asset => asset.status === 'fulfilled' && asset.value)
            console.log(`Successfully uploade ${success.length} new assets`)
        } else {
            console.log(await newAssetPathsResponse.text())
        }
    ...
    async refreshActiveDatacrons() {
        ...
        let templates = await Data.getDatacronTemplates(setIds)

        let focusedTemplates = templates.filter(template => template.focused)

        let activeTemplates = templates.filter(template => template.referenceTemplateId === '' && !template.focused)
        let activeTemplatesMap = listToMap(activeTemplates, 'setId')

        let affixIdList = templates.map(template => template.tier.map(tier => tier.affixTemplateSetId)).flat().flat()
        ...
        let affixTextMap = 
            affixes
            .reduce((arr, affix) => {
                return [...arr, ...affix.affix.map(affix => `${affix.abilityId}:${affix.targetRule}`)]
            }, [])
            .filter(key => key !== ':')
            .reduce((obj, key) => {
                let [abilityId, targetRule] = key.split(':')
                ...
                let value = abilityMap[abilityId]?.replaceAll('{0}', categoryName)
                obj[key] = {categoryId, categoryName, value}
                return obj
            }, {})
        ...
        let datacronData = activeSets.map(set => {
            ...
            return {
                ...
                icon: set.icon,
                focused: focusedTemplates.filter(template => template.setId === setId).reduce((map, template) => {
                    let lastTier = template.tier[template.tier.length - 1]
                    let affixTemplateId = lastTier.affixTemplateSetId[0]
                    let affix = affixesMap[affixTemplateId]
                    let scopeIcon = affix.affix[0].scopeIcon
                    map[template.id] = {
                        id: template.id,
                        focused: template.focused,
                        focusedIcon: template.focusedIcon,
                        levels: template.tier.length,
                        scopeIcon
                    }
                    return map
                }, {}),
        ...
        const { db } = await connectToDatabase()
        try {
            await db.collection('data').updateOne({type: 'datacron'}, {$set: {type: 'datacron', datacron: datacronData, affixTextMap}}, {upsert: true})
    ...
    async refreshActiveEra() {
            let eraDefinition = await Data.getCurrentEra({_id: 0})
            let recipeIdList = eraDefinition?.eraLevel?.map(level => level.upgradeRecipeId)?.filter(id => id !== '')
            let recipesList = await Data.getRecipes(recipeIdList || [])
            let recipeMap = listToMap(recipesList)

            let mysteryBoxIdList = eraDefinition?.coliseum?.bossDefinition?.reduce((arr, boss) => {
                let bossTier = boss.bossRewardTable.reduce((arr, tier) => {
                    let idList = tier.bossProgressionRewardPreview
                        .filter(reward => reward?.detailedReward?.every(it => it.type === 14))
                        .map(reward => reward.detailedReward.map(it => it.id)).flat()
                    return [...arr, ...idList]
                }, [])
                return [...arr, ...bossTier]
            }, [])
            let mysteryBoxesList = await Data.getMysteryBoxes(mysteryBoxIdList || [])
            let mysteryBoxMap = listToMap(mysteryBoxesList)

            const { db } = await connectToDatabase()
            try {
                await db.collection('data').updateOne({type: 'era'}, {$set: {type: 'era', eraDefinition, recipeIdList, mysteryBoxIdList, recipeMap, mysteryBoxMap}}, {upsert: true})
                console.log('active era data updated')
            } catch(err) {
                throw handleDBError(err, 'data', 'update')
            }
    }
