...
class Refresh {
    ...
    async refreshGuild(guildId, projection = defaultGuildProjection) {
        ...
        try{
            ...
            let playerData = await DB.getPlayerArenas(playerIds, {allyCode: 1}, 'playerId')
    ...
    async refreshGameData(latestGamedataVersion = undefined) {
        const { db } = await connectToDatabase()
        let gamedataVersion = latestGamedataVersion || (await Comlink.metaData())['latestGamedataVersion']
        let partitionMap = {
