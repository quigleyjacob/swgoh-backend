...
export async function validateMhannResponse(response, errorMessage, successFieldName = undefined) {
    ...
    if(response.ok) {
        ...
        if(responseBody.code === 0) {
            successFieldName = successFieldName || Object.keys(responseBody).find(key => key !== 'code')
            ...
            if(responseBody.message.includes('not registered')) {
                throw new MyError(401, responseBody.message)
            } else {
                throw new MyError(400, responseBody.message)
            }
        }
    } else {
        let responseBody
        try {
            ...
            let error = new MyError(500, errorMessage, responseText)
            throw error
        }
        if(responseBody.code === 1) {
            throw new MyError(500, responseBody.message, responseText)
        }
    }
