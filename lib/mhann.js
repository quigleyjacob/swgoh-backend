...
class Mhann {
    ...
    async getActiveRaid(allyCode) {
        let body = {
            payload: {
                allyCode
            }
        }
        let response = await fetch(`${this.url}/api/activeraid`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'api-key': this.apiKey},
            body: JSON.stringify(body)
        })
        let activeRaid = await validateMhannResponse(response, 'Error retrieving active raid data from Mhann service', 'data')
