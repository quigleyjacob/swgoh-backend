...
class DB {
    ...
    async refreshPlayer(payload) {
        ...
        comlinkResponse.lastRefreshed = Date.now()

        //computes total GP
        comlinkResponse.galacticPower = comlinkResponse.rosterUnit.reduce((a,b) => a + b.gp, 0)

        // compute GAC Power Score
        comlinkResponse.gacPowerScore = (powerRankingMultiplier[comlinkResponse.playerRating.playerRankStatus.leagueId][comlinkResponse.playerRating.playerRankStatus.divisionId]) * (comlinkResponse.playerRating.playerSkillRating.skillRating) / (comlinkResponse.galacticPower / 1e5)
    ...
    async sessionIsGuildOfficer(sessionId, guildId) {
        // return true // uncomment before pushing changes
        let user = await this.sessionToDiscord(sessionId)
    ...
    async getCurrentGACBoard(session, allyCode) {
        ...
        return {
            ...
            home: gacBoard.gac.home.zones.map(zone => zone.squads.map(squad => squad.team.squad.split(','))),
            away: gacBoard.gac.away.zones.map(zone => zone.squads.map(squad => squad.team.squad.split(',')))
        }
    }
