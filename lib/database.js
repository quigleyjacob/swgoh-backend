...
class DB {
    ...
    async getGuildMemberDiscordRegistrations(guildId) {
        ...
        let guildAllyCodes = guild.member.map(member => member.allyCodes)
    ...
    async getAccountsByDiscordId(discordId) {
        ...
        let allyCodes = accounts.map(account => account.allyCode).sort((a,b) => a.default ? -1 : b.default ? 1 : 0)

        let accountsData
        try {
            accountsData = await db.collection('player').find({allyCodes: {$in: allyCodes}}, {projection: {_id: 0, allyCode: 1, name: 1, guildId: 1}}).toArray()
        } catch(err) {
