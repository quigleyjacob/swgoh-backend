...
export async function getDetailedGuildData(guildId, update, projectionName) {
    ...
    playerData.forEach(player => {
        ...
        player.rosterUnit = player.rosterUnit.reduce((map, obj) => (map[obj.defId] = obj, map), {})
...
export async function refreshGuild(allyCode) {
    ...
    // let { result, err, warning } = await swapi.fetchGuild(payload)
    // if (err) {
    //     return err
    // }
    ...
    return `Guild [${guild.profile.name}] updated`
}
...
export async function refreshUnits() {
    ...
    await Promise.all(units.map(async unit => {
        await db.collection('units').updateOne({ baseId: unit.baseId }, { $set: unit }, { upsert: true })
    }))
