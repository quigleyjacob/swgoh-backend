...
class DB {
    ...
    async getAccountsByDiscordId(discordId) {
        ...
        let accountsData
        try {
            accountsData = await db.collection('player').find({playerId: {$in: accountsIds}}, {projection: {_id: 0, allyCode: 1, name: 1, guildId: 1, playerId: 1}}).toArray()
        } catch(err) {
    ...
    async getDefaultAccount(discordId) {
        ...
        try {
            ...
            account = await db.collection('player').findOne({playerId: discordRegistration.playerId})
        } catch(err) {
    ...
    async sessionIsGuildOfficer(sessionId, guildId) {
        ...
        let guilds = await this.getGuildsByDiscordId(user.id)
        let guild = guilds.filter(guild => guild.guildId === guildId)
        if(guild.length === 0 || account.length === 0) {
            return false
        }
        let guildData = await this.getGuildData(guildId)
        let member = guildData.member.filter(member => member.playerId === account[0].playerId)
        if(member.length === 0) {
            return false
        }
    ...
    async getCommands(guildId, type) {
        const { db } = await connectToDatabase()
        let response
        try {
            response = await db.collection('command').find({guildId: guildId, type: type}).toArray()
        } catch(err) {
    ...
    async addCommand(commandId, type, title, description, guildId) {
        ...
        try {
            if(commandId) {
                response = await db.collection('command').updateOne({_id: commandId}, {$set: command})
            } else {
        ...
        let id = response.insertedId || response.upsertedId
        return await db.collection('command').findOne({_id: id})
    }
...
function handleDBError(err, obj, action) {
    if(err.code = 11000) {
        ...
        return new MyError(500, `There was an error in the database: [action=${action},obj=${obj}] to the database.`, err)
    }
}
