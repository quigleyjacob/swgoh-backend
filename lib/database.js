...
class DB {
    ...
    async getGuildsByDiscordId(discordId) {
        ...
        let guildData
        try {
            guildData = await db.collection('player').find({allyCode: {$in: allyCodes}}, {projection: {_id: 0, allyCode: 1, guildName: 1, guildId: 1}}).toArray()
        } catch(err) {
            throw handleDBError(err, 'Guild', 'get')
        }
        return guildData
    ...
    async refreshBattleTargetingRule() {
        const { db } = await connectToDatabase()
        let data = await Comlink.getBattleTargetingRule()
        try {
