...
class DB {
    ...
    async getCurrentGACBoard(session, allyCode) {
        ...
        return {
            ...
            home: gacBoard.gac.home.zones.map(zone => zone.squads.map(squad => squad.units.map(unit => unit.baseId))),
            away: gacBoard.gac.away.zones.map(zone => zone.squads.map(squad => squad.units.map(unit => unit.baseId)))
        }
    }
