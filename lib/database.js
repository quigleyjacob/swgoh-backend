...
import { player, player as _player } from '../utils/payloads.js'
...
class DB {
    ...
    //Player functions
    async getPlayerData(payload, refresh, projectionName) {
        const { db } = await connectToDatabase()
        if (refresh) {
            await this.refreshPlayer(payload)
        }
        let response = await db.collection('player').findOne(payload, {projection: projections[projectionName]})
        if (!response) {
            throw new MyError(404, `Player [${JSON.stringify(payload)}] not found in the database`, response)
        }
        return response
    }
    ...
    async getPlayableUnits(nameFragment = '', projection={_id: 0, nameKey: 1, baseId: 1}, limit = 0) {
        const { db } = await connectToDatabase()

        let units
        try {
            units = await db.collection('units').find({nameKey: { $regex: nameFragment, $options: 'i'}, obtainableTime: "0", rarity: 7}, { projection: projection}).limit(limit).toArray()
        } catch(err) {
            throw new MyError(500, `Unable to get units [name=${nameFragment}] from the database`, err)
        }
    ...
    async setDefaultAccount(discordId, playerId) {
        console.log(playerId, discordId)
        const { db } = await connectToDatabase()
        let accounts
        try {
            accounts = await db.collection('discord_registration').find({discord_id: discordId}).toArray()
    ...
    async unregisterUser(payload) {
        ...
        if(!toBeRemoved) {
            throw new MyError(500, `Error unregistering, Cannot find user account [playerId=${playerId}]`, err)
        }

        // cannot remove default accounts while nondefault are registered
        if (toBeRemoved.default && nonDefaultCount) {
            throw new MyError(500, `Cannot remove main account while alt account exist [numAlts=${nonDefaultCount}]`, err)
        }
    ...
    try {
        ...
        console.log(err)
        throw new Error(500, "Unable to update image data in the database", err)
    }
