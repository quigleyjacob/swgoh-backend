...
class DB {
    ...
    async getAccountsData(array, projection, key='allyCode', retry = 0) {
        const { db } = await connectToDatabase()
    ...
    async getMaterial() {
        ...
        let response
        try {
            response = await db.collection("material").find({}, {projection: {_id: 0, id: 1, descKey: 1, nameKey: 1, iconKey: 1}}).toArray()
        } catch(err) {
    ...
    async getEquipment() {
        ...
        let response
        try {
            response = await db.collection("equipment").find({}, {projection: {_id: 0, id: 1, mark: 1, nameKey: 1, iconKey: 1}}).toArray()
        } catch(err) {
