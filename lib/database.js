...
class DB {
    ...
    async getPlayableUnits(nameFragment = '') {
        const { db } = await connectToDatabase()

        let units
        try {
            units = await db.collection('units').find({nameKey: { $regex: nameFragment, $options: 'i'}, obtainableTime: "0", rarity: 7}, { projection: {_id: 0, nameKey: 1, baseId: 1}}).limit(20).toArray()
        } catch(err) {
