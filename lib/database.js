...
class DB {
    ...
    async getGuildMemberDiscordRegistrations(guildId) {
        ...
        try {
            ...
            throw new MyError(500, `Error getting discord registrations of guild [guildId=${guildId}]`, err)
        }
    ...
    async setDefaultAccount(discordId, allyCode) {
        ...
        try {
            ...
            throw new MyError(500, `Unable to set default account [allyCode=${allyCode}]`, err)
        }
    ...
    async getDefaultGuild(discordId) {
        ...
        try {
            ...
            throw new MyError(500, `Unable to get default guild [discordId=${discordId}`, err)
        }
        return guildData
    }
    ...
    async unregisterUser(payload) {
        ...
        if(!toBeRemoved) {
            throw new MyError(500, `Error unregistering, Cannot find user account`)
        }
    ...
    async registerServer(serverId, build) {
        ...
        if(build === 'guild') {
            if(!new Array('458371209491644417', '281454605986955264').includes(serverId)) {
                throw new MyError(401, 'You cannot register for this type of server')
            }
        }
        try {
