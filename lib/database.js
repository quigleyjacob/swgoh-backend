...
class DB {
    ...
    async getArenasData(array, projection, key='allyCode') {
        ...
        try {
            ...
            if(remaining.length === 0) {
                ...
                console.log(`missing accounts: ${remaining.length} [${remaining.join(',')}]`)
                await Promise.allSettled(remaining.map(async allyCode => {
                    // let payload = key === 'allyCode' ? {allyCode: id} : {playerId: id}
    ...
    async getActiveDatacrons() {
        ...
        let activeTemplatesMap = new Map();
        // activeTemplates.reduce((map, obj) => (map[obj.setId] = map[obj.setId], map), {})
        activeTemplates.forEach(template => {
            if(activeTemplatesMap[template.setId]) {
                activeTemplatesMap[template.setId].push(template)
            } else {
                activeTemplatesMap[template.setId] = [template]
            }
        })
        ...
        return activeSets.map(set => {
            let setId = set.id
            let activeTemplates = activeTemplatesMap[String(setId)]
            return {
                id: setId,
                name: localizationMap[set.displayName],
                icon: set.icon,
                tier: activeTemplates.map(activeTemplate => {
                    return activeTemplate.tier.map((tier, index) => {
