...
class DB {
    ...
    async getGuildData(guildId, refresh=false, detailed=false, projection={}) {
        ...
        if(detailed) {
            ...
            let playerData
            try {
                playerData = await db.collection('player').find({playerId: {$in: playerAllyCodes}}, {projection: projection}).toArray()
            } catch(err) {
    ...
    async refreshSkills() {
        const { db } = await connectToDatabase()
        let skills = await Comlink.getSkills()
    ...
    async sessionIsGuildOfficer(sessionId, guildId) {
        ...
        let member = guildData.member.filter(member => member.allyCode === account[0].allyCode)
        if(member.length === 0) {
            return false
        }
