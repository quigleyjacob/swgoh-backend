    ...
    side.duelStatus.forEach(zone => {
      zone.warSquad.forEach(squad => {
        squad.squad.cell.forEach(unit => {
          ...
          loadingTasks.push({
            data: { baseId },
            promise: getCanvasImage(`charui/${imageId}.png`, "swgoh-images")
          });
        });
      });
    });
...
export async function drawFullBoard(board, characterMap, player, opponent, renderMode = 'both') {
    ...
    const mode = board.tournamentMapId.includes('5v5') ? 5 : 3
    ...
    if (renderMode === "home") {
        drawBoardSide(ctx, home, imagesMap, characterMap, player, "home", mode);
    } else if (renderMode === "away") {
        drawBoardSide(ctx, away, imagesMap, characterMap, opponent, "away", mode);
    } else {
        ctx.save();
        ctx.translate(0, 0);
        drawBoardSide(ctx, home, imagesMap, characterMap, player, "home", mode);
        ...
        drawBoardSide(ctx, away, imagesMap, characterMap, opponent, "away", mode, offsetX);
        ctx.restore();
    }
...
export function drawBoardSide(ctx, sideStatus, imagesMap, characterMap, account, side, mode, offset = 0) {
    ...
    sideStatus.duelStatus.forEach(zone => {
        ...
        zone.warSquad.forEach((squad, i) => {
            ...
            squad.squad.cell.forEach((unit, unitIndex) => {
                ...
                drawUnitBadges(ctx, innerX, innerY, PORTRAIT_MAX_WIDTH, PORTRAIT_MAX_HEIGHT, unit, isDead, account, imagesMap, characterMap, mode);
...
function drawUnitBadges(ctx, x, y, w, h, unit, isDefeated, account, imagesMap, characterMap, mode) {
  ...
  let accountUnit = account.rosterUnit.find(toon => toon.baseId === baseId)
    ...
    const activeOmiModes = [GAC_MODE, mode === 5 ? GAC_5V5_MODE : GAC_3V3_MODE]
    let activeOmis = activeOmiModes.reduce((total, mode) => {
        total += accountUnit?.omicron?.activatedByMode?.[mode]?.length || 0
        return total
    }, 0)

  let zetaCount = accountUnit.zetaCount
  let omicronCount = accountUnit?.omicron ? activeOmis : accountUnit.omicronCount
  ...
  if (omicronCount > 0) {
    ctx.drawImage(imagesMap["omicron"], rightX+2, rightY, omiSize, omiSize)
    drawCenteredBadgeText(ctx, omicronCount,  x + (w - textSize), y + h - textSize / 2, textSize)
  }
    if (relicTier > 0) {
